...
impl fmt::Display for TicketNewError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let err_msg = match self {
            TicketNewError::TitleError(err) => format!("{err}"),
            TicketNewError::DescriptionError(err) => format!("{err}"),
        };
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket_result {
        Ok(ticket) => ticket,
        Err(err) => match err {
            TicketNewError::DescriptionError(_) => Ticket { title: title, description: "Description not provided".to_string(), status: status },
